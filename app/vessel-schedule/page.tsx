...
type WordLikeRow = { rowIndex: number; byPort: Record<string, ScheduleStop | undefined> }
...
export default function VesselSchedulePage() {
  ...
  const wordLikeRows = useMemo<WordLikeRow[]>(() => {
    const rows: WordLikeRow[] = []
    const routeCodes = route.map((r) => r.portCode)

    for (const stop of schedule) {
      const rowIndex = route.length === 0 ? 0 : Math.floor(stop.index / route.length)
      if (!rows[rowIndex]) {
        rows[rowIndex] = {
          rowIndex,
          byPort: Object.fromEntries(routeCodes.map((code) => [code, undefined])),
        }
      }
      rows[rowIndex].byPort[stop.portCode] = stop
    }
  ...
  const periodTitle = useMemo(() => {
    ...
    const monthLabel = `${start.toLocaleDateString("ru-RU", { month: "long" })} – ${end.toLocaleDateString("ru-RU", { month: "long" })}`
    return `${monthLabel} ${start.getFullYear()} года`
  }, [months, startDate])
  ...
  const exportCsv = () => {
    ...
    for (const rp of route) {
      const name = portMap[rp.portCode]?.name ?? rp.portCode
      headerTop.push(`${name} Приход`, `${name} Отход`)
    }

    const rows = wordLikeRows.map((row) => {
      const line = [String(row.rowIndex + 1)]
      for (const rp of route) {
        const stop = row.byPort[rp.portCode]
        line.push(stop ? formatDate(stop.eta) : "", stop ? formatDate(stop.etd) : "")
      }
      return line
    })

    const content = [headerTop, ...rows]
    ...
    a.href = url
    a.download = `word-like-schedule-${selectedVesselId}.csv`
    a.click()
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1700px] flex-col gap-6 px-4 py-8">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Формат таблицы приближен к Word-шаблону: порты в шапке и отдельные колонки «Приход/Отход».
          </p>
        </div>
      ...
      <section className="rounded-lg border p-4">
        <h2 className="text-center text-lg font-semibold">Расписание движения судов ООО «Амур Инвест»</h2>
        <p className="text-center text-sm">{periodTitle}</p>
        <p className="mb-3 text-center text-lg font-semibold italic">{selectedVessel.name}</p>

        <div className="overflow-x-auto">
          <table className="w-full min-w-[1400px] border-collapse text-sm">
            <thead>
              <tr className="border bg-muted/20 text-center">
                <th className="border px-2 py-1" rowSpan={2}>
                  №
                </th>
                {route.map((rp) => (
                  <th key={`h1-${rp.portCode}`} className="border px-2 py-1" colSpan={2}>
                    {portMap[rp.portCode]?.name ?? rp.portCode}
                  </th>
                ))}
              </tr>
              <tr className="border bg-muted/10 text-center">
                {route.flatMap((rp) => [
                  <th key={`arr-${rp.portCode}`} className="border px-2 py-1">
                    Приход
                  </th>,
                  <th key={`dep-${rp.portCode}`} className="border px-2 py-1">
                    Отход
                  </th>,
                ])}
              </tr>
            ...
            <tbody>
              {wordLikeRows.map((row) => (
                <tr key={row.rowIndex} className="text-center">
                  <td className="border px-2 py-1">{row.rowIndex + 1}</td>
                  {route.flatMap((rp) => {
                    const stop = row.byPort[rp.portCode]
                    return [
                      <td key={`arr-${row.rowIndex}-${rp.portCode}`} className="border px-2 py-1 whitespace-nowrap">
                        {stop ? formatDate(stop.eta) : ""}
                      </td>,
                      <td key={`dep-${row.rowIndex}-${rp.portCode}`} className="border px-2 py-1 whitespace-nowrap">
                        {stop ? formatDate(stop.etd) : ""}
                      </td>,
                    ]
                  })}
                </tr>
      ...
      <section className="rounded-lg border p-4">
        <h2 className="mb-3 text-lg font-semibold">Детализация и ручная правка ETA</h2>
        <div className="overflow-x-auto">
